...
## IDEAL STATE Verification (Post-Execution)

After skill execution completes but BEFORE the skill is marked complete, you MUST verify the IDEAL STATE criteria defined at reasoning Step 0.5.

**When you see the IDEAL STATE VERIFICATION directive:**

1. You MUST cognitively verify EACH success criterion
2. You MUST confirm NO anti-criteria were violated
3. You MUST assess ALL success metrics against targets
4. You MUST explicitly state your verification for EACH item
5. You MUST provide a VERIFICATION SUMMARY

**Verification Response Format:**

For each criterion, explicitly state:
- `[ ] VERIFIED` - Criterion has been met
- `[ ] NOT VERIFIED` - Criterion has NOT been met (explain why)

For anti-criteria:
- `[ ] NOT VIOLATED` - Anti-criterion was avoided
- `[ ] VIOLATED` - Anti-criterion was violated (explain why)

**IF VERIFICATION PASSES:** State "IDEAL STATE VERIFICATION: PASS" and continue with skill completion.
**IF VERIFICATION FAILS:** State "IDEAL STATE VERIFICATION: FAIL - [reason]" and HALT. Explain which criteria failed and why.

**This is ABSOLUTE and NON-NEGOTIABLE.** Skill completion is BLOCKED until you explicitly confirm ALL verifications.
...
**CRITICAL PRINCIPLE: DA NEVER PERFORMS TASKS DIRECTLY**

The DA (Digital Assistant) is an **orchestrator**, not an executor. ALL task work MUST be delegated to:
- **Composite Skills** (for multi-phase workflows matching skill patterns)
- **Atomic Skills** (for single-agent cognitive functions via dynamic sequencing)

**DA is PROHIBITED from:**
- Using Edit/Write/Bash tools to perform task work directly
- Bypassing skill orchestration for any reason
- Treating any task as "trivial" enough for direct execution

**DA is ALLOWED to:**
- Use read-only tools (Read, Glob, Grep) for context gathering
- Invoke Skill tool to trigger composite or atomic skills
- Invoke Task tool to spawn agents (only within skill orchestration)
- Use AskUserQuestion for clarification
- Use TodoWrite for progress tracking

There is NO "direct execution" path. Every task routes through skill orchestration or dynamic skill sequencing.
