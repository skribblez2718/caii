...
Execute a search using the Tavily API and return formatted results with source attribution.

**Usage:** `/search:tavily-search [your search query]`
...
if [ -z "$QUERY" ]; then
  ...
  echo "Usage: /search:tavily-search [your search query]"
  exit 1
fi
