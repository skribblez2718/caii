...
TWO-ROUTE EXECUTION MODEL:
All tasks route through one of two execution paths:

1. SKILL ORCHESTRATION - For tasks matching formal skill patterns
   - Invokes composite skills with defined phase sequences
   - Each phase triggers orchestrate-* atomic skills
   - Structured multi-phase workflow with memory files

2. DYNAMIC SKILL SEQUENCING - For tasks requiring cognitive work without matching skills
   - Invokes orchestrate-* atomic skills in sequence
   - Flexible agent sequencing for novel task patterns
   - Same cognitive quality as skill orchestration
