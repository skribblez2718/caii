...
def validate_tool_call(tool_name: str, tool_input: Dict[str, Any]) -> int:
    ...
    if is_orchestration_active(state):
        if not is_allowed_da_tool(tool_name):
            return EXIT_BLOCK

    # Not in active orchestration - still restrict DA from Edit/Write/Bash
    if tool_name in DA_BLOCKED_TOOLS_DURING_ORCHESTRATION:
        print(f"[ENFORCEMENT] DA attempted {tool_name} outside orchestration context", file=sys.stderr)
        print(f"[ENFORCEMENT] DA should use Skill tool or Task tool instead", file=sys.stderr)
        return EXIT_BLOCK
