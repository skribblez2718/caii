"""
initialize-caii-session.py
==========================

Main CAII session initialization hook that runs at the start of every Claude Code
session.
...
* Relies on ``load-core-context.ts`` (configured elsewhere) to inject CAII context
...
1. Set environment variables in ``settings.json``:
   ...
   - ``VOICE_SERVER_PROTOCOL``: Protocol for the voice server (defaults to ``http``)
   ...
   - ``CAII_DIRECTORY``: Path to your CAII directory (defaults to ``$HOME/.claude``)
...
def test_stop_hook() -> bool:
    ...
    * When the hook exists and is executable, sets the initial terminal tab
    ...
    """
    caii_dir = os.environ.get("CAII_DIRECTORY", str(Path.home() / ".claude"))
    stop_hook_path = Path(caii_dir) / ".claude" / "hooks" / "stop" / "stop.py"
...
def main() -> int:
    ...
    * Sends a low-priority "systems initialized" notification (if a local
    ...
    try:
        ...
        # Note: CAII core context loading is handled by load-core-context.py hook
