...
def main() -> None:
    parser = argparse.ArgumentParser(description="BUILD Phase (Step 4)")
    ...
    if not state.start_phase(STEP_NUM):
        print("ERROR: Cannot transition to BUILD phase", file=sys.stderr)
    ...
    content = load_content(__file__, "build_phase.md")
