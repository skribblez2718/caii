...
def main() -> None:
    parser = argparse.ArgumentParser(description="LEARN Phase (Step 8.5)")
    ...
    if not state.start_phase(STEP_NUM):
        print("ERROR: Cannot transition to LEARN phase", file=sys.stderr)
    ...
    content = load_content(__file__, "learn_phase.md")
