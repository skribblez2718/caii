...
def main() -> None:
    parser = argparse.ArgumentParser(description="OBSERVE Phase (Step 1)")
    ...
    if not state.start_phase(STEP_NUM):
        print("ERROR: Cannot transition to OBSERVE phase", file=sys.stderr)
    ...
    content = load_content(__file__, "observe_phase.md")
