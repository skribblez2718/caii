...
def main() -> None:
    parser = argparse.ArgumentParser(description="PLAN Phase (Step 3)")
    ...
    if not state.start_phase(STEP_NUM):
        print("ERROR: Cannot transition to PLAN phase", file=sys.stderr)
    ...
    content = load_content(__file__, "plan_phase.md")
