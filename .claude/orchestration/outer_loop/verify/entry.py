...
def main() -> None:
    parser = argparse.ArgumentParser(description="VERIFY Phase (Step 8)")
    parser.add_argument("--state", required=True, help="Session ID for state tracking")
    parser.add_argument("task_id", nargs="?", default="", help="Task ID (legacy)")
    ...
    if not state.start_phase(STEP_NUM):
        print("ERROR: Cannot transition to VERIFY phase", file=sys.stderr)
    ...
    content = load_content(__file__, "verification.md")
    prompt = substitute_placeholders(
        content,
        user_query=state.user_query,
        session_id=state.session_id,
        task_id=state.session_id,  # For backward compatibility
    )
    print(prompt)
