...
10-step mandatory reasoning protocol (Step 0, 0.5, 1-8) that runs BEFORE any task execution. This is the entry point for ALL user queries.

**Step 0 (Johari Window Discovery)** executes at the START of every interaction to transform unknown unknowns into known knowns.

**Step 0.5 (IDEAL STATE Capture)** defines explicit success criteria, anti-criteria, and verification methods using "The Last Algorithm" methodology before formal reasoning begins.
...
│                              ▼                                               │
│                   ┌──────────────────┐                                       │
│                   │    Step 0.5      │  ← IDEAL STATE Capture (NEW)          │
│                   │  IDEAL STATE     │    (0-5 cycles until >= 95% complete) │
│                   │   Capture        │                                       │
│                   └────────┬─────────┘                                       │
│                            │                                                 │
│                            ▼                                                 │
...
│                 │       │              ┌────────┐              │        │   │
│                 │       │              │ Step 8 │◄─────────────┘        │   │
│                 │       │              │Knowledge│  (verifies against   │   │
│                 │       │              │Transfer │   IDEAL STATE)       │   │
│                 │       │              └────┬────┘                      │   │
│                 │       │                   │                           │   │
│                 │       │     ┌─────────────┼─────────────┐             │   │
│                 │       │     │             │             │             │   │
...
│   ├── step_0_5_ideal_state_capture.py # Step 0.5 - IDEAL STATE Capture (NEW)
...
│   └── step_{n}_{name}.md   # e.g., step_0_johari_discovery.md, step_0_5_ideal_state_capture.md
...
TRANSITIONS = {
    ...
    JOHARI_DISCOVERY      → [IDEAL_STATE_CAPTURE, HALTED],  # Step 0.5 or HALT
    IDEAL_STATE_CAPTURE   → [SEMANTIC_UNDERSTANDING, IDEAL_STATE_CAPTURE, HALTED],  # Loop, proceed, or HALT
...
STATE_TO_STEP = {
    ...
    IDEAL_STATE_CAPTURE: 0.5,   # Step 0.5 - Success criteria definition (NEW)
...
| `ideal_state` | object | IDEAL STATE definition (NEW - see below) |
| `ideal_state_iteration` | int | Current IDEAL STATE iteration (0-5) |
| `ideal_state_max_iterations` | int | Max allowed iterations (default: 5) |
| `ideal_state_history` | list | History of IDEAL STATE refinements |

### IDEAL STATE Object (NEW)

| Field | Type | Description |
|-------|------|-------------|
| `success_criteria` | list | List of success criteria with id, criterion, verifiable, verification_method, priority |
| `anti_criteria` | list | List of anti-criteria (things to avoid) with id, criterion, verifiable, verification_method |
| `success_metrics` | list | List of metrics with id, name, target_value, measurement_method |
| `completeness_score` | float | 0.0-1.0 indicating completeness (>= 0.95 = sufficient) |
| `exit_condition` | object | Description and minimum_completeness for task completion |
| `sufficiency` | object | is_sufficient (bool), confidence (high/medium/low), gaps (list) |
...
**Critical Rule:** If ANY clarifying questions exist after Step 0, HALT and ask before proceeding to Step 0.5.

## Step 0.5: IDEAL STATE Capture (NEW)

Step 0.5 implements "The Last Algorithm" methodology by iteratively refining explicit success criteria before formal reasoning begins. This ensures verifiable success criteria guide all downstream reasoning.
...
|-----------|-------------|
| **Success Criteria** | What MUST be true for success (must-have, should-have, nice-to-have priorities) |
| **Anti-Criteria** | What MUST NOT happen (explicit failure conditions) |
| **Success Metrics** | Quantifiable measures with target values |
| **Exit Condition** | Precise definition of when task is complete |
...
| Anti-Criteria Coverage | 15% |
| Metrics Completeness | 15% |

**Critical Rule:** Step 0.5 can HALT for clarification, ITERATE for refinement, or PROCEED to Step 1 when >= 95% complete.

### Post-Execution Verification (NEW)

IDEAL STATE criteria are verified at skill completion via `ideal_state_verifier.py`:

| Integration Point | Description |
|-------------------|-------------|
| **Skill Completion** | `common_skill_complete.py` loads IDEAL STATE via `execution_session_id` |
| **Verification Directive** | Printed to stdout, Claude MUST cognitively verify each criterion |
| **Blocking** | Skill cannot be marked complete until verification confirmed |
| **Enforcement** | Uses ABSOLUTE/NON-NEGOTIABLE/MANDATORY language hierarchy |

**Flow:**
1. Skill execution completes (all phases done)
2. `_verify_ideal_state_criteria()` loads reasoning state
3. If IDEAL STATE exists, verification directive printed
4. Claude explicitly verifies each criterion (VERIFIED/NOT VERIFIED)
5. Claude provides VERIFICATION SUMMARY (PASS/FAIL)
6. Only on PASS: Skill marked complete

**Files:**
- `protocols/skill/ideal_state_verifier.py` - Builds verification directive
- `protocols/skill/composite/common_skill_complete.py` - Invokes verification checkpoint
