...
# Step configuration
# Protocol has 10 steps: Step 0, 0.5, 1-8 (Johari Discovery + IDEAL STATE + Steps 1-8)
TOTAL_STEPS: Final[int] = 10  # 0, 0.5, 1-8

# STEP_NAMES uses float keys to support Step 0.5
STEP_NAMES: Final[dict[float, str]] = {
    0: "JOHARI_DISCOVERY",
    0.5: "IDEAL_STATE_CAPTURE",  # NEW: Phase 1 - The Last Algorithm integration
...
# STEP_TITLES uses float keys to support Step 0.5
STEP_TITLES: Final[dict[float, str]] = {
    0: "Johari Window Discovery",
    0.5: "IDEAL STATE Capture",  # NEW: Phase 1 - The Last Algorithm integration
...
# Note: STEP_CONTENT_PATTERN and STEP_SCRIPT_PATTERN are deprecated
# Use get_step_content_path() and get_step_script_path() which handle
# float step numbers like 0.5 (formatted as "0_5" in filenames)
...
def get_step_content_path(step_num: float) -> Path:
    """Get the path to a step's markdown content file.

    Args:
        step_num: Step number (supports floats like 0.5 for Step 0.5)
    """
    step_name = STEP_NAMES.get(step_num, "unknown").lower()
    # Handle float step numbers (e.g., 0.5 -> "0_5")
    step_num_str = str(step_num).replace(".", "_")
    return CONTENT_DIR / f"step_{step_num_str}_{step_name}.md"


def get_step_script_path(step_num: float) -> Path:
    ...
    # Handle float step numbers (e.g., 0.5 -> "0_5")
    step_num_str = str(step_num).replace(".", "_")
    return STEPS_DIR / f"step_{step_num_str}_{step_name}.py"
...
def format_mandatory_directive(command: str, context: str = "") -> str:
    ...
    from shared.directives.base import _format_directive_core
