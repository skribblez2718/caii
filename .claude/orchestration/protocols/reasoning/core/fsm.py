...
The FSM enforces the 10-step reasoning protocol (Step 0, 0.5, 1-8) with:
- Step 0: Johari Window Discovery (ambiguity detection at START of every interaction)
- Step 0.5: IDEAL STATE Capture (iterative success criteria refinement, 0-5 cycles)
...
class ReasoningState(Enum):
    ...
    Step 0.5 (IDEAL STATE Capture) iteratively refines success criteria.
    ...
    IDEAL_STATE_CAPTURE = auto()         # Step 0.5 - Success criteria definition (NEW)
...
class ReasoningFSM:
    ...
    TRANSITIONS: Dict[ReasoningState, List[ReasoningState]] = {
        ...
        ReasoningState.JOHARI_DISCOVERY: [
            ReasoningState.IDEAL_STATE_CAPTURE,  # Normal flow to Step 0.5
            ReasoningState.HALTED,               # HALT if critical questions in Johari
        ],
        ReasoningState.IDEAL_STATE_CAPTURE: [
            ReasoningState.SEMANTIC_UNDERSTANDING,  # Proceed when sufficient (>=95%)
            ReasoningState.IDEAL_STATE_CAPTURE,     # Loop for next iteration
            ReasoningState.HALTED,                  # HALT if needs user clarification
        ],
    ...
    STATE_TO_STEP: Dict[ReasoningState, float] = {
        ...
        ReasoningState.IDEAL_STATE_CAPTURE: 0.5,       # Step 0.5 (NEW)
