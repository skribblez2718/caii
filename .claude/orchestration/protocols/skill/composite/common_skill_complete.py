...
from skill.ideal_state_verifier import build_verification_directive, has_ideal_state
...
def _verify_ideal_state_criteria(state: SkillExecutionState) -> Optional[str]:
    """
    Generate IDEAL STATE verification directive if IDEAL STATE exists.

    This function loads the reasoning protocol state linked via execution_session_id
    and builds a verification directive if IDEAL STATE criteria were defined.

    Args:
        state: The skill execution state

    Returns:
        Verification directive string, or None if no IDEAL STATE
    """
    if not state.execution_session_id:
        return None

    try:
        # Import reasoning state - use fully-qualified import
        from reasoning.core.state import ProtocolState

        reasoning_state = ProtocolState.load(state.execution_session_id)

        if not reasoning_state or not has_ideal_state(reasoning_state):
            return None

        return build_verification_directive(state, reasoning_state)
    except (ImportError, Exception) as e:
        # Log error but don't fail skill completion
        print(f"INFO: Could not load IDEAL STATE for verification: {e}", file=sys.stderr)
        return None
...
def skill_complete(skill_name: str) -> None:
    ...
    # IDEAL STATE verification checkpoint (MANDATORY)
    # This checkpoint loads the IDEAL STATE from reasoning session and forces
    # Claude to cognitively verify each success criterion before skill completion.
    # Verification is cognitive, NOT automated - Claude must explicitly confirm.
    verification_directive = _verify_ideal_state_criteria(state)
    if verification_directive:
        print(verification_directive)
        # Skill completion continues after Claude confirms verification
