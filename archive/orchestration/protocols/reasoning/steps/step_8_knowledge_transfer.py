...
2. Verifies routing aligns with IDEAL STATE criteria from Step 0.5
3. Implements the SHARE/PROBE/MAP/DELIVER framework for clarification
4. Triggers loop-back to Step 4 if contradiction detected
"""
...
class Step8KnowledgeTransfer(BaseStep):
    ...
    def get_extra_context(self) -> str:
        """
        Display iteration context and IDEAL STATE for verification.
        Prior step content is in conversation context.
        """
        context_parts = []

        # Iteration info
        iteration, max_iter, route = self.get_iteration_info()
        context_parts.append(f"ROUTING VALIDATION ITERATION: {iteration} of {max_iter}")
        context_parts.append(f"PRELIMINARY ROUTE: {route}")
        context_parts.append("")

        # IDEAL STATE verification context
        ideal_state = self.state.ideal_state
        if ideal_state:
            context_parts.append("## IDEAL STATE Verification Context")
            context_parts.append("")
            context_parts.append("**Verify routing decision against IDEAL STATE criteria:**")
            context_parts.append("")

            # Success criteria
            success_criteria = ideal_state.get("success_criteria", [])
            if success_criteria:
                context_parts.append("### Success Criteria to Verify")
                for sc in success_criteria:
                    priority = sc.get("priority", "unknown")
                    context_parts.append(f"- [{priority.upper()}] {sc.get('criterion', 'N/A')}")
                context_parts.append("")

            # Anti-criteria
            anti_criteria = ideal_state.get("anti_criteria", [])
            if anti_criteria:
                context_parts.append("### Anti-Criteria (Must Not Violate)")
                for ac in anti_criteria:
                    context_parts.append(f"- {ac.get('criterion', 'N/A')}")
                context_parts.append("")

            # Exit condition
            exit_cond = ideal_state.get("exit_condition", {})
            if exit_cond:
                context_parts.append(f"### Exit Condition")
                context_parts.append(f"{exit_cond.get('description', 'Not specified')}")
                context_parts.append("")

            context_parts.append("**Validation Question:** Does the routing decision align with IDEAL STATE criteria?")
            context_parts.append("")
