...
{skill}/complete.py
    └→ skill_complete(skill_name)  # from common_skill_complete.py
        ...
        ├→ state.complete()
        ├→ state.save()
        ├→ _verify_ideal_state_criteria(state)  # ⚠️ MANDATORY verification checkpoint
        │   ├→ Load IDEAL STATE from reasoning session via execution_session_id
        │   ├→ Print verification directive if IDEAL STATE exists
        │   └→ Claude MUST cognitively verify each criterion before proceeding
        └→ print("{SKILL_NAME}_COMPLETE")
```
