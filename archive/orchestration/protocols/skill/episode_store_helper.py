...
def create_episode_from_bypass_execution(
    task_description: str,
    outcome: str = "success",
    tools_used: Optional[List[str]] = None,
) -> Episode:
    """
    Create an episode from bypass mode execution.

    Used for tasks that bypass the reasoning protocol (-b flag).
    ...
    Returns:
        Episode object ready to be saved
    """
    task_id = f"bypass-{datetime.now().strftime('%Y%m%d%H%M%S')}"

    return Episode(
        episode_id=generate_episode_id(task_id, "bypass-task"),
        task_id=task_id,
        timestamp=datetime.now().isoformat(),
        skill_name="bypass-task",
        task_description=task_description,
        task_keywords=extract_keywords(task_description),
...
def store_bypass_episode(
    task_description: str,
    outcome: str = "success",
    tools_used: Optional[List[str]] = None,
) -> str:
    """
    Convenience function to store a bypass mode episode.
    ...
    Returns:
        Episode ID of the stored episode
    """
    episode = create_episode_from_bypass_execution(
        task_description=task_description,
        outcome=outcome,
